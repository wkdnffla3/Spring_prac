...
class EmpDeptTests {
	...
	@Test
	void test3(){
		List<Emp> list = empRopository.findByEnameContainingAndJobContaining("N", "N");
	...
	void 부서검색(){
		...
		List<Emp> list = empRopository.findByDept(dept);
		System.out.println(list);
	}
	...
	@Test
	void testCode(){
		List<Emp> list = empRopository.findByEname("KING");

		System.out.println(list); // 순환 참조 조심
	}
