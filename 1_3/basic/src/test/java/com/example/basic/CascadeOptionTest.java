...
class CascadeOptionTest {
	...
	@Test
	void cascade_persist_적용시_부모_저장하면_자식도_저장된다() {
		// Post post = new Post();
	...
	@Test
	void cascade_merge_적용시_부모수정시_자식도_자동_merge된다() {
	}

	@Test
	void cascade_remove_적용시_부모_삭제시_자식도_삭제된다() {
	...
	void contextLoads2() {
		...
		list.forEach((team) -> {
			...
			// System.out.printf("%s %s %s" , team.getTeamName(), team.getTeamName(),
			// team.getTeamName());
